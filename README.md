...
Observe that Temporal Web reflects the workflow, but it is still in "Running" status. This is because there is no Workflow or Activity Worker yet listening to the `SIGN_UP_TASK_QUEUE` task queue to process this work.
...
Now you can see the workflow run to completion. You can also see the worker polling for workflows and activities in the task queue at [http://localhost:8088/namespaces/default/task-queues/SIGN_UP_TASK_QUEUE](http://localhost:8088/namespaces/default/task-queues/SIGN_UP_TASK_QUEUE).
